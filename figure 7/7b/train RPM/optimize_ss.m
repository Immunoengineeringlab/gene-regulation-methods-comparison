...
lb_cp=[0 0 0 0  2 2 2 2  0 0 0 0 ]';     %lower bound of continuous parameters
ub_cp=[1 1 1 1  10 10 10 10 1 1 1 1 ]';   %upperbound of continuous parameters
A = [];
b = [];
Aeq = [];
beq = [];
...
while k<100
    ...
    [x,fvalu] = ga(fun,noofvariables,A,b,Aeq,beq,lb,ub,nonlcon,IntCon,options);
        D=toc
    k=k+1;
    gatecombs_rpm_networkB(k,:)=x;
    Fvalue_rpm_networkB(k,:)=fvalu;
        tiempo(k)=D;
    save('workspace_gatecombinations_rpm_networkB') 
