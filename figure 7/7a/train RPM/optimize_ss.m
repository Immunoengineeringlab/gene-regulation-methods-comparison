...
kom(1:length(data),1:2)=1; % to assign knockouts. here no KO is assigned to any node.


warning('off')


inputcounts=[3 3]; %number of input edges for each node in the order [A B]
...
lb_cp=[0 0 2 2 0 0]';     %lower bound of continuous parameters
ub_cp=[1 1 10 10 1 1]';   %upperbound of continuous parameters
A = [];
b = [];
Aeq = [];
beq = [];
lb_dp=[1 1]';     %lower bound of discrete parameters representing logic gatges for each node
ub_dp=UB';   %upperbound of discrete parameters representing logic gatges for each node
lb=[lb_dp;lb_cp ];
ub=[ub_dp;ub_cp ];
IntCon = [1 2];               % indices of parameters that takes only integer values
nonlcon = [];
options = gaoptimset('UseParallel',true,'display','iter'); % set to "false" if parallel evaluations are not desired
...
while k<200
    ...
    [x,fvalu] = ga(fun,noofvariables,A,b,Aeq,beq,lb,ub,nonlcon,IntCon,options);
        D=toc
    k=k+1;
    gatecombs_rpm_networkA(k,:)=x;
    Fvalue_rpm_networkA(k,:)=fvalu;
        tiempo(k)=D;
    save('workspace_gatecombinations_rpm_networkA')  
