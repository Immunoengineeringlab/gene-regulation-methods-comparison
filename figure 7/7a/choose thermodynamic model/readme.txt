...
since thermodynamic model for networks with high in degrees take longer to compute. user can stop the optimization as soon as four steadystates are reached denoted by objective function values being less than -40(iteratively displayed).
corresponding optimized parameter can be found in "SaveBest.mat " where all the best result for each generation is saved.

run "data_training.m" to get training data. "trainingdataA.mat" will be saved at the end of the run.
