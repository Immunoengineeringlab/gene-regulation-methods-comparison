load('workspace_gatecombinations_hillcube_networkB','gatecombs','yyy')
load('dataforprediction_networkB','initial','timedata')
kom=[1,1,1,1];
...
for x=1:1:length(gatecombs(:,1))
    ...
    parfor i=1:1:length(initialconditions(:,1))
        ...
        [cost,ticota] = predictionerror(gatecombs(x,:),data,yyy,initial,kom);
        FVw(i)=cost;
        stvw(i,:)=ticota{1}(end,:);
    end
...
save('hillcubepredictionerrors_networkB')